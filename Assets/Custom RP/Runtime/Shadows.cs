...
public class Shadows
{
	...
	void RenderDirectionalShadows() 
	{
		...
		//开启ShadowPancaking,ShadowPancaking只对正交视图起作用
		//方向光的光源摄像机是正交视图
		buffer.SetGlobalFloat(shadowPancakingId, 1f);
		buffer.BeginSample(bufferName);//TODO buffer的使用有点忘记了,需要复习并记笔记
		ExecuteBuffer();
	...
	void RenderOtherShadows()
	{
		...
		buffer.ClearRenderTarget(true, false, Color.clear);
		//关闭ShadowPancaking,ShadowPancaking只对正交视图起作用
		//点光源和聚光灯的光源摄像机不是正交视图
	...
	void RenderSpotShadows(int index, int split, int tileSize)
	{
		...
		//计算阴影偏移量，避免因为距离增大而ShadowMap一个像素所代表的面积增大所带来的，阴影粉刺
