...
//通过滤波器采样阴影贴图以达到soft shadow的效果（原理同bloom）
...
float SampleOtherShadowAtlas(float3 positionSTS, float3 bounds)//STS = shadow texture space
{
...
float FilterOtherShadow(float3 positionSTS, float3 bounds)
{
		...
		for (int i = 0; i < OTHER_FILTER_SAMPLES; i++) {
			shadow += weights[i] * OTHER_FILTER_SAMPLES(
